...
"""Remove duplicate same-material flat top surface coverage, without offsets.
...
def top_cleanup(mesh,region_xy=None,top_z=None,cell_mm=1.):
    ...
    for start in range(0,len(quads),100000):
        ...
        keep=(np.ptp(v[:,:,2],axis=1)==0)&np.all(n[:,:,2]>.999999,axis=1)
        ...
        ids.extend((np.flatnonzero(keep)+start).tolist())
    ...
    for count,idx in enumerate(ids):
        ...
        # Positive top quads produced by bead_mesh are convex in the XY plane.
        cross=[]
        for a,b,c in zip(poly,poly[1:]+poly[:1],poly[2:]+poly[:2]):
            cross.append((b[0]-a[0])*(c[1]-b[1])-(b[1]-a[1])*(c[0]-b[0]))
        if min(cross)<-EPS:
            raise ValueError(f'Nonconvex top quad {idx}; triangulate it before coplanar cleanup')
        ...
        cells=[(plane,mat,x,y)for x in range(math.floor(bbox[0]/cell_mm),math.floor(bbox[2]/cell_mm)+1)for y in range(math.floor(bbox[1]/cell_mm),math.floor(bbox[3]/cell_mm)+1)]
    ...
    # top quads are replaced with triangulated convex pieces on the same plane.
    keep=np.ones(len(quads),bool);newv=[];newt=[];newm=[];sourceq=[]
    for idx,fragments in changed.items():
        keep[idx]=False;z=float(vertices[quads[idx,0],2])
        for poly in fragments:
            start=len(vertices)+len(newv)
            newv.extend([[p[0],p[1],z]for p in poly])
            for i in range(1,len(poly)-1):
                newt.append([start,start+i,start+i+1]);newm.append(material[idx]);sourceq.append(idx)
    ...
    result['vertex_normals']=np.concatenate((normals,np.tile(np.asarray([[0,0,1]],np.float32),(len(newv),1))))
    ...
    report={'candidate_top_quads':len(ids),'tested_bbox_pairs':pairs,'changed_top_quads':len(changed),
            ...
            'scope':'Surface coverage union is preserved in XY; coincident duplicate top coverage is subtracted. Independent component watertightness is intentionally not asserted after removing internal duplicate surfaces.'}
